...
const [openNodes, setOpenNodes] = useState<Record<string, boolean>>({});
  ...
  const filteredMappingLeaves = useMemo(() => {
    const q = mappingSearchText.trim().toLowerCase();

    if (!q) return [];

    return allMappingLeaves.filter((mappingLeaf) =>
      [
        mappingLeaf.mappingCode,
        mappingLeaf.label,
        displayMappingText(mappingLeaf.label, entityKind),
        mappingLeaf.statement,
        mappingLeaf.section,
        displayMappingText(mappingLeaf.section, entityKind),
        ...mappingLeaf.path,
        ...mappingLeaf.path.map((item) => displayMappingText(item, entityKind)),
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        ...
        <section style={styles.rightPane}>
          ...
          <div style={styles.searchWrap}>
            <input
              ...
              onChange={(event) => setMappingSearchText(event.target.value)}
              placeholder="Search mapping code or description..."
            />
          </div>

          <div style={styles.libraryScroll}>
            {mappingSearchText.trim() ? (
              filteredMappingLeaves.length ? (
                filteredMappingLeaves.map((mappingLeaf) => {
                  const usage = leafUsage[mappingLeaf.mappingCode];
                  const isSelected = selectedLeaf?.id === mappingLeaf.id;
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        ...(usage && !isSelected ? styles.treeLeafUsed : {}),
                      }}
                      onClick={() => selectMappingLeaf(mappingLeaf)}
                    >
                      <span>
                        <strong>{mappingLeaf.mappingCode}</strong>
                        {" · "}
                        {displayMappingText(mappingLeaf.label, entityKind)}
                        <small style={styles.mappingSearchPath}>
                          {[
                            displayMappingText(mappingLeaf.section, entityKind),
                            ...mappingLeaf.path.map((item) =>
                              displayMappingText(item, entityKind),
                            ),
                          ]
                            .filter(Boolean)
                            .join(" > ")}
                        ...
                        {usage ? (
                          <small style={styles.usageText}>
                            Used by {usage.count} account
                            {usage.count === 1 ? "" : "s"}
                          </small>
                        ) : null}
                      </span>
                      <small>{mappingLeaf.leadScheduleNumber}</small>
                    </button>
                  );
                })
              ) : (
                <div style={styles.mappingSearchEmpty}>
                  No mapping codes or descriptions match this search.
                </div>
              )
            ) : (

...
import { buildAfsPrintStatementEngine } from "../components/AfsPrintStatementEngine";
...
export default function AfsPrintStudioPage() {
  ...
  const narrativeContext = {
   ...
   const statementEngine = useMemo(
    () => buildAfsPrintStatementEngine(trialBalanceLines),
    [trialBalanceLines]
  );
  ...
  const detailedIncomeRows = statementEngine.detailedIncomeRows;
  ...
  function buildAccountingPolicyPrintItems() {
    ...
    accountingPolicySections.forEach((section) => {
      ...
      const groupLabel = section.groupLabel || section.label;
                  ...
                  {buildAccountingPolicyPrintItems().map((item, index) => {
                    ...
                    return (
                      <div key={item.section.key}>
                        <h2 style={sectionHeadingStyle()}>
                          {index + 1}. {current.title || item.section.label}
                        </h2>
              ...
              <AfsA4Page {...reportHeaderProps}>
                ...
                >
                  ...
                  {noteSections
                    ...
                    .map((section, index) => {
                      const current =
                        activeNoteTexts[section.key] ||
                        defaultNoteTexts[section.key] || {
                          title: section.label,
                          text: "",
                        };
                      ...
                      return (
                        <div key={section.key}>
                          <h2 style={sectionHeadingStyle()}>
                            {index + 1}. {current.title || section.label}
                          </h2>
              ...
              <AfsA4Page {...reportHeaderProps}>
                ...
                >
                  ...
                  <p style={paragraphStyle()}>
                    Tax computation is switched on. The next step connects the
                    existing tax calculator values and assessed loss fields from
                    Client Setup.
                  </p>
                </section>
              </AfsA4Page>

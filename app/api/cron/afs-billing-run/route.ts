...
async function generateRun(request: Request) {
  ...
  if (existingRun && rebuild) {
    const { data: existingBatches, error: existingBatchesError } = await supabase
      ...
      .select("id,status,invoice_id,invoice_number")
      .eq("billing_run_id", existingRun.id);

    if (existingBatchesError) throw existingBatchesError;

    const invoicedBatch = (existingBatches ?? []).find(
      (batch) =>
        Boolean(batch.invoice_id) ||
        Boolean(batch.invoice_number) ||
    ...
    if (invoicedBatch) {
      return NextResponse.json(
        {
          error:
            "This billing run already contains an invoiced batch and cannot be rebuilt.",
        },
        { status: 409 }
      );
    }
    ...
    // Remove only subscription events generated by this run so the corrected
    // run can recreate them with the current user count/rates.
  ...
  try {
    ...
    for (const organisation of activeOrganisations) {
      ...
      /*
       * Some existing PP organisations selected an AFS plan before
       * afs_plan_activated_at was introduced. Do not skip those customers.
       * afs_billing_enabled + afs_plan is the billing eligibility gate.
       * When an activation date exists, we still respect a future activation.
       */
      const activationDate = organisation.afs_plan_activated_at
        ? startOfDayUtc(new Date(organisation.afs_plan_activated_at))
        : null;

      if (activationDate && activationDate > subscriptionPeriodEnd) {
        continue;
      }

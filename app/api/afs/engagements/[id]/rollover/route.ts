...
function isRetainedIncomeLine(line: AnyRow) {
  /*
    Mapping-code only. Retained income / accumulated loss is mapping 810
    in the AFS mapping library. Never classify this by account name, label,
    path or description.
  */
  ...
  return (
    mappingCode === "810" ||
    mappingCode.startsWith("810.") ||
    mappingCode === "800.810" ||
    mappingCode.startsWith("800.810.")
  );
}
...
function buildRolloverTrialBalanceLine(
  ...
  const sourceFinalBalance = roundMoney(getFinalBalance(sourceLine));
...
async function createNewRollover(
  ...
  try {
    const organisationId = String(sourceEngagement.organisation_id || "").trim();

    if (!organisationId) {
      throw new Error("The source engagement is not linked to a practice.");
    }
    ...
    if (printStudioResult.data) {
      ...
      const settingsInsert = {
        ...
        /*
          A new Next Flight must start with no fabricated equity movements.
          These values are explicit rather than relying on legacy/default
          Print Studio state.
        */
        statement_overrides: {
          scePriorOtherMovements: 0,
          sceCurrentOtherMovements: 0,
          sceOtherMovements: 0,
        },
...
export async function POST(
  ...
  try {
    ...
    console.error("NEXT FLIGHT ROLLOVER ERROR:", error);

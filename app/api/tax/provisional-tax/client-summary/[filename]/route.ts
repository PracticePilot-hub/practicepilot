...
async function renderPdf(
  ...
  try {
    const { filename: filenameParam } = await context.params;
    const requestedFilename = decodeURIComponent(filenameParam || "");
...
export async function GET(
  ...
  try {
    ...
    if (!payload) {
      return NextResponse.json(
        { success: false, error: "Missing PDF payload." },
        { status: 400 }
      );
    }

    const body = JSON.parse(payload);
    return renderPdf(req, context, body);
  } catch (error: any) {
    return NextResponse.json(
      {
        success: false,
        error: error?.message || "Unable to read PDF payload.",
      },
      { status: 400 }
    );
  }

...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  if ("quoteFilePath" in body || "quoteFileName" in body) {
    const filePath = String(body.quoteFilePath || "").trim();
    const fileName = String(body.quoteFileName || "").trim();

    if (!filePath || !fileName) {
      return NextResponse.json({ error: "Quote file path and name are required" }, { status: 400 });
    }

    const { data: quoteFile, error: quoteFileError } = await supabase
      .from("project_line_item_quote_files")
      .insert([
        {
          project_id: id,
    ...
    if (quoteFileError) {
      return NextResponse.json({ error: quoteFileError.message }, { status: 500 });
    }

    return NextResponse.json({ quoteFile });
  }
...
export async function PUT(req: Request, context: RouteContext) {
  ...
  const overrideType = overrideAmount === null ? null : String(body.overrideType || "manual");
...
export async function DELETE(req: Request, context: RouteContext) {
  ...
  if (body.action === "delete-quote-file") {
    const quoteFileId = String(body.quoteFileId || "").trim();

    if (!quoteFileId) {
      return NextResponse.json({ error: "Quote file ID is required" }, { status: 400 });
    }
